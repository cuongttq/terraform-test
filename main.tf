...
resource "openstack_lb_members_v2" "webapp_members" {
  pood_id = openstack_lb_pool_v2.webapp_pool.id

  member {
    address = openstack_compute_instance_v2.webapp_instance.address
    protocol_port = 80
  }

  member {
    address = openstack_compute_instance_v2.webapp_instance_1.address
    protocol_port = 80
  }
}
...
resource "openstack_networking_floatingip_associate_v2" "webapp_floatingip_associate" {  
  ...
  port_id = openstack_lb_listener_v2.webapp_listener.vip_port_id
}
  
output "webapp_public_ip" {  
  value       = openstack_networking_floatingip_v2.webapp_floatingip.address  
  description = "Web Application URL"  
}  
  
output "webapp_vip" {  
  value       = openstack_lb_listener_v2.webapp_listener.vip_address  
